...
"""Module providing the `RandomTickData` class for generating synthetic tick data."""

from vectorbtpro import _typing as tp
from vectorbtpro.base.accessors import BaseIDXAccessor
from vectorbtpro.data.custom.random import RandomData
...
class RandomTickData(RandomData):
    """Data class for synthetic tick data generation with randomized timestamps.

    This class extends `vectorbtpro.data.custom.random.RandomData` to generate data with non-uniform
    time intervals between data points, simulating realistic tick data timing.

    See:
        * `RandomTickData.fetch_key` for argument details.

    !!! info
        For default settings, see `custom.random_tick` in `vectorbtpro._settings.data`.
    """

    _settings_path: tp.SettingsPath = dict(custom="data.custom.random_tick")
