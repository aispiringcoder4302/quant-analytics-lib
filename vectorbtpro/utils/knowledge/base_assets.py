...
__all__ = [
    ...
    "IPythonDisplayer",
    "IPythonMarkdownDisplayer",
    "IPythonHTMLDisplayer",
    "HTMLFileDisplayer",
    "KnowledgeAsset",
]
...
ChatContentProcessorT = tp.TypeVar("ChatContentProcessorT", bound="ChatContentProcessor")


class ChatContentProcessor(Configured):
    """Class for processing any chat content.

    Used as `display_format="plain"` in `KnowledgeAsset.chat`."""
    ...
    def __enter__(self: ChatContentProcessorT) -> ChatContentProcessorT:
        self.initialize()
        return self
...
class IPythonDisplayer(ChatContentProcessor):
    """Class for displaying any content in IPython.

    Used as `display_format="ipython_text"` in `KnowledgeAsset.chat`."""

    def __init__(self, *args, **kwargs) -> None:
        ChatContentProcessor.__init__(self, *args, **kwargs)
    ...
    def initialize(self) -> None:
        ChatContentProcessor.initialize(self)
    ...
    def update(self, final: bool = False) -> None:
        ChatContentProcessor.update(self, final=final)
        self.update_display()


class IPythonMarkdownDisplayer(IPythonDisplayer):
    """Class for displaying Markdown content in IPython.
...
class IPythonHTMLDisplayer(IPythonDisplayer):
...
class HTMLFileDisplayer(ChatContentProcessor):
...
class KnowledgeAsset(Configured, MutableSequence):
    ...
    def chat(
        ...
        system_as_user: tp.Optional[bool] = None,
        context_prompt: tp.Optional[str] = None,
        display_format: tp.Union[None, str, ChatContentProcessor] = None,
        output_to: tp.Optional[tp.Union[str, tp.TextIO]] = None,
        flush_output: tp.Optional[bool] = None,
        buffer_output: tp.Optional[bool] = None,
        close_output: tp.Optional[bool] = None,
        ...
        * A subclass of `ChatContentProcessor`
        ...
        if isinstance(display_format, str):
            ...
            if display_format.lower() == "plain":
                display_format = ChatContentProcessor(
            ...
            elif display_format.lower() == "ipython_text":
                display_format = IPythonDisplayer(
                    output_to=output_to,
                    flush_output=flush_output,
                    buffer_output=buffer_output,
                    close_output=close_output,
            ...
            elif display_format.lower() == "ipython_markdown":
                display_format = IPythonMarkdownDisplayer(
                    output_to=output_to,
                    flush_output=flush_output,
                    buffer_output=buffer_output,
                    close_output=close_output,
            ...
            elif display_format.lower() == "ipython_html":
                display_format = IPythonHTMLDisplayer(
                    output_to=output_to,
                    flush_output=flush_output,
                    buffer_output=buffer_output,
                    close_output=close_output,
            ...
            elif display_format.lower() == "html":
                display_format = HTMLFileDisplayer(
                    output_to=output_to,
                    flush_output=flush_output,
                    buffer_output=buffer_output,
                    close_output=close_output,
        ...
        checks.assert_instance_of(display_format, ChatContentProcessor, "display_format")
        ...
        if stream:
            ...
            content = meta["full_content_lambda"](response)
        ...
        if isinstance(display_format, HTMLFileDisplayer) and display_format.file_handle is not None:
